...
| 🔴 Alta | Generación de código de estructura `SharedMessagePool` | Medio |Implementar en el generador la traducción de la estructura de comunicación `SharedMessagePool` (agentes que publican y consumen sobre un pool común de mensajes) al grafo de LangGraph. |
| 🔴 Alta | Generación de código de estructura `Decentralized` | Medio | Implementar en el generador la traducción de la estructura de comunicación `Decentralized` (comunicación directa entre agentes sin coordinador) al grafo de LangGraph. |
| 🟡 Media | Invocación del grafo | Medio | El generador produce un bloque de invocación y comprobación de resultados completamente provisional. El objetivo es que el código generado tenga calidad de producción con integración de LangSmith. |
| 🟡 Media | Mecanismo de inicio y fin del grafo | Medio | Para la estructura `Layered` el inicio y fin se infieren correctamente a partir de las referencias `next`. Sin embargo, hace falta implementar algún mecanismo más para que cuando se mezclen distintas estructuras de comunicaición el generador sepa cuando generar los edges hacia start y end. |
